# Power Analysis — Somebunny Brought the Whole Warren (Baylen, the Haymaker, B4)
...
### Role Key
Ramp · Fix · Token (makes tokens) · Mult (token doubler/tripler) · Payoff (turns width into a win) · Anthem · Draw · Removal · Protect · Recur · Tutor · Land
...
|------|------|:-----:|------|
| Baylen, the Haymaker | Payoff / Engine | 7 | Tap X untapped tokens for X mana, to draw, or to pump all attackers +1/+1 — the engine that converts the rabbit flood into ramp, cards, and a lethal alpha strike. Not a combo piece and doesn't need to stick (the win lines live in the 99), but he's the fair-overrun deck's throttle. Cursed Totem taxes his ability but never turns the deck off. |

---

## Token engine — the core (17 cards)

> The whole deck is "make a wide board, then multiply and overrun it." These are the bodies that build the crowd.

| Card | Role | Power | Note |
|------|------|:-----:|------|
| Hare Apparent ×10 | Token | 9 | The core. Each new Hare makes a 1/1 Rabbit for every *other* Hare you control, so copies snowball — and with a doubler out it's absurd. Ten copies is a single critical role filled ten times: elite redundancy that reads as a virtual tutor package. |
| Warren Warleader | Token / Anthem | 7 | Attack trigger: a tapped, already-attacking Rabbit *or* a team +1/+1. Body, token maker, and finisher rolled together. |
| Head of the Homestead | Token | 6 | Two 1/1 Rabbits on ETB — cheap, immediate width. |
| Farmer Cotton | Token | 6 | X 1/1 Halflings **and** X Food on ETB, scaling with your mana — an instant crowd that doubles under the multipliers, with Food to feed Pawpatch Formation and buffer your life total. |
| Harvestrite Host | Token / Draw | 6 | Pumps a guest on each Rabbit ETB and draws a card the second time a Rabbit arrives each turn. |
| Jacked Rabbit | Token | 6 | Attacks to make Rabbits equal to its power; Ravenous pours excess mana into counters and a card. A one-card army and a mana sink. |
| Valley Questcaller | Anthem / Support | 6 | Static +1/+1 to your other Rabbits (plus Bird/Bat/Mouse) and a scry per arrival — a lord that also smooths draws. |
| Regal Bunnicorn | Payoff body | 6 | Power/toughness each equal your nonland permanents; in this shell it's the biggest thing on the board without evasion. |
...
## Go-wide finishers & payoffs (6)

| Card | Role | Power | Note |
|------|------|:-----:|------|
| Craterhoof Behemoth | Payoff / Finisher | 9 | The overrun. On a wide board it ends the game the turn it resolves. |
| Finale of Devastation | Payoff / Tutor / Finisher | 9 | Fetches Craterhoof (or any creature) to the battlefield, and at X≥10 it *is* the overrun with haste. Doubles as the deck's most flexible tutor. |
| Jetmir, Nexus of Revels | Payoff | 7 | Grants vigilance, then trample, then double strike as the crowd grows — turns a wide-but-gentle board into a lethal one. |
| Caretaker's Talent | Payoff / Draw | 7 | Class enchantment: draw on token ETBs → copy your best token → a +2/+2 team anthem. Card advantage *and* a finisher gear — and it triggers once per token event, so it dodges the counter-per-token trigger bloat that made Cathars' Crusade a real-life/Forge pace problem. |
| Halo Fountain | Alt-win / Utility | 6 | An untap engine with Baylen and Seedborn; untap fifteen creatures and it simply wins the game. Do not cut (untap synergy + alt-win). |
| Impact Tremors | Payoff / Drain | 5 | One damage to each opponent per creature ETB — with the flood and doublers it quietly drains the table without a single attack. |
...
| Card | Role | Power | Note |
|------|------|:-----:|------|
| Second Harvest | Token | 7 | Doubles *all* your existing tokens at instant speed — an explosive one-card board expansion before a Craterhoof swing. |
| Secure the Wastes | Token | 7 | Instant-speed X 1/1 Warriors on demand — a standalone flood that scales with your mana, dodges sorcery-speed wraths, and rebuilds the board after a sweep; doubles under every multiplier and feeds Baylen and Halo Fountain a fresh crowd. |
...
| Chrome Mox | Ramp | 6 | Fast but card-disadvantage; kept for the early speed the deck wants to hit its curve-out turns. |
| Utopia Sprawl | Ramp + Fix | 6 | T1 enchant-a-Forest for an extra pip of any color — lands a doubler a full turn early. |

---

## Tutors & consistency (6)

| Card | Role | Power | Note |
|------|------|:-----:|------|
| Survival of the Fittest | Tutor engine | 8 | A repeatable creature tutor every single turn — the deck's best consistency piece, finding a doubler-payoff, a body, or Craterhoof on demand. |
| Chord of Calling | Tutor | 8 | Instant-speed, convoke, creature straight to the battlefield — ambush a piece or flash in Craterhoof. |
| Eladamri's Call | Tutor | 7 | Any creature to hand at instant speed; guaranteed access to the right body. |
| Enlightened Tutor | Tutor | 7 | Finds the noncreature engine — a doubler, Skullclamp, The Great Henge, or Sylvan Library. |
| Worldly Tutor | Tutor | 6 | Any creature to the top; pairs with Enlightened toward a real premium tutor base and lifts the Consistency tutor column. |
| Crop Rotation | Land tutor | 6 | Instant-speed Gaea's Cradle (for a huge mana turn) or Boseiju (to answer a hoser). |
...
| Deflecting Swat | Protect / Removal | 6 | Free redirect of a spell or ability while the commander's out — the closest a blueless deck gets to a counterspell. |
| Pawpatch Formation | Removal | 6 | Modal: kill a flyer, blow up an enchantment, or make a Food and draw. |

---

## Protection (4 board-level)

| Card | Role | Power | Note |
|------|------|:-----:|------|
| Teferi's Protection | Protect | 9 | Phase the whole warren out — dodges wraths, exile, and even a Cyclonic Rift; the premier "you can't ruin this." |
| Heroic Intervention | Protect | 7 | Hexproof + indestructible for your board at instant speed. |
| Flawless Maneuver | Protect | 7 | Free team indestructible while the commander's out. |
| Clever Concealment | Protect | 7 | Phase out any number of your permanents (convoke) — the board-wipe insurance a go-wide deck actually needs. |

> Plus the token spells **Grand Crescendo** and **For the Common Good** carry indestructible riders, giving the deck **five-plus** board-level protection effects — deep enough to read at the top of the Resilience protection column.
...
By deliberate design (a "fair" go-wide beatdown), the deck runs **no assembled infinite combo** — the win is a real, interactive board that overruns through Craterhoof, Finale, or Jetmir, with Halo Fountain and Impact Tremors as alternate closes. This caps the Resilience score on the combat path rather than the combo path, which is the intended trade-off.
...
1. **Nykthos → a basic** to shore up against nonbasic hate.
2. **A third rebuild engine** (e.g., Sun Titan — recurs Skullclamp, Impact Tremors, and a Hare each turn) if Resilience still reads low.
3. **Trim a doubler for a body** only if hands keep flooding on multipliers.

**Bottom line:** mechanically cohesive and powerful — one of the more *consistent* and *protected* fair go-wide decks you'll build, with a redundant token core, multiplicative payoffs, and a fast overrun. Its ceiling is capped only by the intentional no-combo rule and a thin basic count; both are design choices, not build errors.
